...
If any changed files are under `pydbus-stubs/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `pydbus-stubs/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
