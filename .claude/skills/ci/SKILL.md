...
If any changed files are under `pydbus-stubs/` or `tests/`, run the following agents **in order**:
...
- **changelog** - update `CHANGELOG.md` with entries for the changes. After it completes, check if
  ...
  the relevant commit. **Only run when changes affect users**: files under `pydbus-stubs/`, `tests/`,
...
For example, if a commit contains `pydbus-stubs/commands/main.py`,
`tests/test_main_command.py`, and `CHANGELOG.md`, the component is determined by
`pydbus-stubs/commands/main.py` and `tests/test_main_command.py` only.
...
- Changes span unrelated components (e.g. `pydbus-stubs/media.py` and `.claude/agents/release.md`).
...
For Python files, strip the `pydbus-stubs/` prefix and replace `/` with `.` (like module imports).

- Python file `pydbus-stubs/media.py` → `media:`.
- Multiple files under `pydbus-stubs/commands/` → `commands:`.
- Single command file `pydbus-stubs/commands/admin.py` → `commands.admin:`.
