...
Fixes mypy type errors and eliminates `Any` usage in the pydbus-stubs project.
...
1. Fix all mypy errors reported by `uv run mypy pydbus-stubs`
...
Place new TypedDict classes in `pydbus-stubs/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`pydbus-stubs/typing.py` when shared.
...
1. Run `uv run mypy pydbus-stubs` and capture all errors.
...
4. Run `uv run mypy pydbus-stubs` again to verify fixes.
