...
@type_check_only
class _OrgBluezAdapter1Dict(TypedDict, total=False):
    Address: str
...
@type_check_only
class _OrgBluezDevice1Dict(TypedDict, total=False):
    Adapter: str
    Address: str
    AddressType: Literal['public', 'random']
    Alias: str
...
@type_check_only
class _OrgBluezInput1Dict(TypedDict, total=False):
    ReconnectMode: str


@type_check_only
class _OrgBluezMedia1Dict(TypedDict, total=False):
    SupportedUUIDs: list[str]


@type_check_only
class _OrgBluezMediaControl1Dict(TypedDict, total=False):
    Connected: bool


@type_check_only
class _OrgBluezBattery1Dict(TypedDict, total=False):
    Percentage: int
    Source: str


@type_check_only
class _OrgBluezGattService1Dict(TypedDict, total=False):
    Device: str
    Handle: int
    Includes: list[str]
    Primary: bool
...
@type_check_only
class _OrgBluezGattCharacteristic1Dict(TypedDict, total=False):
    Flags: list[str]
    Handle: int
    MTU: int
    Service: str
...
@type_check_only
class _OrgBluezGattDescriptor1Dict(TypedDict, total=False):
    Characteristic: str
    Handle: int
    UUID: str
    Value: list[int]
...
@type_check_only
class _OrgBluezLEAdvertisingManager1Dict(TypedDict, total=False):
    ActiveInstances: int
...
@type_check_only
class _OrgBluezNetwork1Dict(TypedDict, total=False):
    Connected: bool
    Interface: str
    UUID: str
...
_OrgBluezDict = TypedDict(
    '_OrgBluezDict',
    {
        ...
        # Dicts below are always empty because they have no properties. However, the key existence
        # may still be useful for introspection.
...
class _OrgBluez(_CompositeObject[_T]):
    def __getitem__(  # type: ignore[override] # ty: ignore[invalid-method-override]
            self,
            key: Literal['org.freedesktop.DBus.ObjectManager']) -> _OrgFreedesktopDBusObjectManager:
        ...
...
class _OrgFreedesktopNotifications(_CompositeObject[_T], Generic[_T, _ST]):
    ...
    def GetCapabilities(self) -> list[str]:
        ...  # We could use Literal[] here, but KDE also returns its own values not in the spec.
...
class ProxyMixin(Generic[_T]):
    ...
    ) -> _OrgFreedesktopNotifications[_T, object]:
        ...
    ...
    def get(self,
            ...
            *,
            timeout: int | None = None) -> _CompositeObject[_T]:
        ...
